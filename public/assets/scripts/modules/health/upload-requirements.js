$(document).ready(function () {
    ...
    if ($(".btn-submit-payment-order").length) {
        ...
        $(".btn-submit-payment-order").on("click", function (e) {
            let payment_details = getPaymentDetails();
            let app_ref_no = $(this).attr("data-ref-no");

            if (!payment_details) {
                _systemAlert("alert", "No Payment Selected!");
                return;
            }

            ajaxRequest(
                "/executor/payment/create/" + app_ref_no,
                { PaymentInformation: JSON.stringify(payment_details) },
                $(this)
            );
        });
    }
...
function enablePaymentOrder() {
    ...
    let approve_text = $(".btn-payment-order").attr("data-status");
    let cntr = 0;
    ...
    if (cntr === status_len) $(".btn-payment-order").attr("disabled", false);
    else $(".btn-payment-order").attr("disabled", true);
}
